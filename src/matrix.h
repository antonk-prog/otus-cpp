...
{
public:
    explicit Matrix(int defaultValue = 0) : m_default(defaultValue) {}
    ...
    int operator()(size_t r, size_t c) const { return get(static_cast<int>(r), static_cast<int>(c)); }

    std::size_t size() const noexcept { return m_store.size(); }

    class iterator {
    public:
        using inner_it = std::unordered_map<long long,int>::const_iterator;
...
private:
    ...
    int m_default{0};
    std::unordered_map<long long,int> m_store;
...
public:
    ...
    public:
        ...
        CellProxy& operator=(int v) { m_.set(r_, c_, v); return *this; }
        operator int() const { return m_.get(r_, c_); }
    private:
        Matrix& m_;
        int r_, c_;
    };
...
inline Matrix::CellProxy Matrix::RowProxy::operator[](size_t c) { return CellProxy(m_, r_, static_cast<int>(c)); }